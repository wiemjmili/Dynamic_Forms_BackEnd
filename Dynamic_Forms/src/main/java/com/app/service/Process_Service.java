...
public class Process_Service {
	...
	public ProcessEngine getEngine() {
		...
		if (processEngine == null) {
			ProcessEngineConfiguration procEngConfig = new StandaloneProcessEngineConfiguration()
					.setJdbcUrl("jdbc:postgresql://localhost:5432/FLOWABLE_DB").setJdbcUsername("postgres")
					.setJdbcPassword("1995").setJdbcDriver("org.postgresql.Driver").setAsyncExecutorActivate(true)
	...
	public Process getProcessById(String idProc) {
	...
	public String getIdDefinition(String idWF, String xmlWF) {
		...
		while (find == false && list.size() > i) {
			if (list.get(i).getWorkflow().getId().equals(idWF)) {
				find = true;
				idDef = list.get(i).getIdDefinition();
			} else {
				i++;
			}
		}
