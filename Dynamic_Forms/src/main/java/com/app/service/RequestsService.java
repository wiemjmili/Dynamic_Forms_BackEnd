...
public class RequestsService {
	...
	public String addRequest(Requests req) {
		...
		req.setUser(user);
		UserTask UT = userTaskService.getUTById(req.getForm().getIdUT());

		ProcessEngine processEngine = process_Service.getEngine();
		Map<String, Object> variables = new HashMap<>();
		RuntimeService runtimeService = processEngine.getRuntimeService();

		String idDef = process_Service.getIdDefinition(UT.getWorkFlow().getId(), UT.getWorkFlow().getXml());

		ProcessInstance processInstance = runtimeService.startProcessInstanceById(idDef, variables);
		...
		for (int j = 0; j < listUT.size(); j++) {
			...
			for (int i = 0; i < listUT.get(j).getGroup().size(); i++) {
				...
				for (int k = 0; k < Current_User.getGroups().size(); k++) {
					...
					if (listUT.get(j).getGroup().get(i).getName().equals(Current_User.getGroups().get(k).getName())) {
						find = true;
					...
					for (int l = 0; l < listUTvalidate.size(); l++) {
						...
						if (listUTvalidate.get(l).getId().equals(listReq.get(j).getForm().getIdUT())) {
							...
							for (int m = 0; m < listUTvalidate.get(l).getGroup().size(); m++) {
								...
								if (listReq.get(j).getUser().getGroups().get(k).getName()
										.equals(listUTvalidate.get(l).getGroup().get(m).getName())) {
									find = true;
								}
							}
						}
					...
					for (int k = 0; k < listRes.get(j).getUser().getGroups().size(); k++) {
						for (int l = 0; l < listUTvalidate.size(); l++) {
							for (int m = 0; m < listUTvalidate.get(l).getGroup().size(); m++) {
								...
								if (listRes.get(j).getUser().getGroups().get(k).getName()
										.equals(listUTvalidate.get(l).getGroup().get(m).getName())) {
									find = true;
								}
							}
